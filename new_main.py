...
async def make_graph_call(graph: Graph, top: int = 5):
    event_list = []
    events = await graph.make_graph_call(top)
...
async def execute_function_call(graph, tool_call):
    func_name = tool_call.function.name 
    if func_name == "get_outlook_caledar_events":
        results = await make_graph_call(graph, top=5)
    else:
        results = f"Error: function {func_name} does not exist"
    return results
